...
def run_neoalchemy_code(
    code: str, driver: GraphDatabase.driver, timeout_seconds: int = 10
) -> Dict[str, Any]:
...
"""
    ...
    try:
        ...
        try:
            ...
            return {"stdout": "", "result": None, "error": f"Syntax error: {str(e)}"}
    ...
    return {"stdout": stdout_capture.getvalue(), "result": formatted_result, "error": error_message}
...
def format_result(result: Any) -> Any:
    ...
    if hasattr(result, "model_dump"):
        ...
        try:
            ...
            try:
                return {
                    k: format_result(v) for k, v in result.__dict__.items() if not k.startswith("_")
                }
            except Exception:
                return str(result)
