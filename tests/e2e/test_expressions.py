...
def test_complex_query_patterns(repo, clean_db):
    ...
    with repo.transaction() as tx:
        ...
        young_high_scorers = (
            tx.query(Person).where(Person.age < 30).where(Person.score >= 90).find()
        )
...
def test_array_multiple_conditions(repo, clean_db):
    ...
    with repo.transaction() as tx:
        ...
        python_developers = (
            tx.query(Person).where("developer" in Person.tags).where("python" in Person.tags).find()
        )
...
def test_between_method(repo, clean_db):
    ...
    with repo.transaction() as tx:
        ...
        middle_aged = tx.query(Person).where(Person.age.between(25, 35)).find()
...
def test_in_list_methods(repo, clean_db):
    ...
    with repo.transaction() as tx:
        ...
        selected_people_1 = (
            tx.query(Person).where(Person.name.in_list(["Alice", "Charlie", "Eve"])).find()
        )
        ...
        selected_people_2 = (
            tx.query(Person).where(Person.name.one_of("Alice", "Charlie", "Eve")).find()
        )
...
def test_chained_comparison(repo, clean_db):
    ...
    with repo.transaction() as tx:
        ...
        medium_scorers = tx.query(Person).where(75 <= Person.score <= 90).find()
        ...
        target_age_range = tx.query(Person).where(25 < Person.age < 40).find()
...
def test_chained_comparison_with_equality(repo, clean_db):
    ...
    with repo.transaction() as tx:
        ...
        # Age >= 25 and age <= 35
        middle_aged_exact = tx.query(Person).where(25 <= Person.age <= 35).find()
        ...
        older_people = tx.query(Person).where(35 <= Person.age).find()
...
def test_combined_pythonic_features(repo, clean_db):
    ...
    with repo.transaction() as tx:
        # Create sample data
        tx.create(
            Person(name="Alice", age=30, email="alice@example.com", tags=["developer", "python"])
        )
        tx.create(Person(name="Bob", age=25, email="bob@test.com", tags=["designer", "ui"]))
        tx.create(
            Person(name="Charlie", age=35, email="charlie@example.com", tags=["developer", "java"])
        )
        tx.create(
            Person(name="David", age=40, email="david@sample.net", tags=["manager", "python"])
        )
        ...
        python_devs_example = (
            tx.query(Person)
            .where("python" in Person.tags)
            .where(Person.email.endswith("example.com"))
            .find()
        )
