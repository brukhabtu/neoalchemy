...
def setup_constraints(
    ...
    if model_classes is None:
        ...
        import neo4j
        if hasattr(neo4j, "__version__") and tuple(map(int, neo4j.__version__.split("."))) >= (4, 4):
...
def _setup_indexes(session, model_class, entity_type, is_node):
    ...
    index_fields = set(model_class.get_indexes()) - unique_fields  # Don't index fields that already have a unique constraint
