...
class Neo4jTransaction:
    ...
    def update(self, model: M) -> M:
        ...
        result = self._tx.run(
            query, 
            {
                "uid": str(uid) if hasattr(uid, "__str__") else uid,
                "data": data
            }
        )
    ...
    def relate(self, from_model: Any, relationship: Any, to_model: Any) -> Dict[str, Any]:
        ...
        rel_type = getattr(relationship.__class__, "__type__", relationship.__class__.__name__.upper())
    ...
    def merge(self, model_class, **properties):
        ...
        existing = self.query(model_class).where(
            getattr(model_class, constraint_field) == properties[constraint_field]
        ).find_one()
